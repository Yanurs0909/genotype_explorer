...
function computeMultiTrait(input) {
  ...
  bloodFatherCands.forEach((f) => bloodMotherCands.forEach((m) => {
    ...
    bloodCombos.push({ father: f, mother: m, phenoDist });
  }));
  ...
  function simpleTrait(fatherPheno, motherPheno) {
    ...
    fCands.forEach((f) => mCands.forEach((m) => {
      combos.push({ father: f, mother: m, phenoDist: phenotypeDistFromCross(f, m) });
    }));
    return { combos };
  }
...
/* ---- PDF로 저장 (브라우저 인쇄 기능 활용 - 서버/외부 라이브러리 없이 동작) ---- */
document.getElementById("pdfBtn").addEventListener("click", () => {
  window.print();
});

/* ---- 결과 공유 (Web Share API, 지원하지 않으면 클립보드 복사) ---- */
document.getElementById("shareBtn").addEventListener("click", async () => {
  if (!lastBasicResults) return;
  const remaining = lastBasicResults.filter((r) => !r.eliminated).map((r) => `${r.father}×${r.mother}`);
  const text = `[Genotype Explorer] "${lastBasicInput.traitName}" 탐색 결과\n최종 가능한 부모 유전자형: ${remaining.join(", ") || "없음"}`;

  if (navigator.share) {
    try {
      await navigator.share({ title: "Genotype Explorer 결과", text });
    } catch {
